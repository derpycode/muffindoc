...
\\ Next, type `\texttt{brew doctor}'. This should tell you ``\texttt{your system is ready to brew}''. If it doesnÕt, see Appendix B. 
...
brew install cmake
brew install gcc
brew install hdf5 --with-fortran %I got an error: no --with-fortran option 
brew install netcdf
brew install wget
\end{verbatim}
...
Verify the netcdf version number in the directory above, it's currently at 4.6.0, i.e.:\\
\vspace{-5pt}\begin{verbatim}
NETCDF_DIR=/usr/local/Cellar/netcdf/4.6.0
\end{verbatim}
